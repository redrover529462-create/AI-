...
def _hero_line(day: str, title: str, story_count: int) -> str:
    return (
        f"VOL.{day} · {story_count} STORIES · MORNING ISSUE\n\n"
        f"# 罗宋汤日报\n\n"
        f"{date.today().strftime('%Y年%m月%d日')}  ·  每日晨间简报"
    )


def briefing_cover_summary(ai_items: list[SourceItem], video_items: list[SourceItem], github_items: list[SourceItem]) -> str:
...
def briefing_keywords(ai_items: list[SourceItem], video_items: list[SourceItem], github_items: list[SourceItem]) -> str:
    keywords: list[str] = []
...
def briefing_one_line(ai_items: list[SourceItem], video_items: list[SourceItem], github_items: list[SourceItem]) -> str:
...
def render_briefing(title: str, ai_items: list[SourceItem], video_items: list[SourceItem], github_items: list[SourceItem]) -> str:
    ...
    sections = [
        ...
        f"### 封面摘要\n{briefing_cover_summary(ai_items, video_items, github_items)}",
        f"### 本期关键词\n{briefing_keywords(ai_items, video_items, github_items)}",
        f"### 一句话总评\n{briefing_one_line(ai_items, video_items, github_items)}",
        '---',
