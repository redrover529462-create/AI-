...
from .render import briefing_cover_summary, briefing_keywords, briefing_one_line, render_briefing
...
def _wait_until_send_time(timezone_name: str) -> None:
    local_now = datetime.now(tz=ZoneInfo(timezone_name))
    workflow_schedule = os.getenv("GITHUB_EVENT_SCHEDULE", "").strip()
    if workflow_schedule != "0 4 * * 1-5":
        return
    target = local_now.replace(hour=12, minute=0, second=0, microsecond=0)
    if local_now >= target:
        return
    delay_seconds = (target - local_now).total_seconds()
    if delay_seconds > 0:
        print(f'waiting {delay_seconds:.0f}s until {target.strftime("%H:%M:%S")} {timezone_name} before sending')
...
def main(argv: list[str] | None = None) -> int:
    ...
    _wait_until_send_time(config.timezone)
